changequote(`{{', `}}')
define({{CURRENT_DATE}}, {{esyscmd(date -u +"%a %b %d %R %Z" | tr -d '\n')}})
    ...
    chart = new Chart(ctx, {
      ...
      options: {
        ...
        plugins: {
          ...
          title: {
            display: true,
            text: "Updated on CURRENT_DATE. No new papers on arXiv on Sat & Sun",
          },
        },
        elements: {
          point: {
  ...
  function handleMouseMove(event) {
    ...
    if (chart) {
      try {
        ...
        console.log('Error getting chart elements:', error);
        hideTooltip();
      }
    document.addEventListener("DOMContentLoaded", function () {
  const ctx = document.getElementById("umap-plot").getContext("2d");
  ...
  function handleMouseClick(event) {
    if (chart) {
      try {
        ...
        console.log('Error handling click event:', error);
      }
    }
  }
